...
class TestAccountModel:
    ...
    def test_create_account(self, app):
        """Test creating a new account"""
        with app.app_context():
            user = User(microsoft_id="test", email="test@test.com", name="Test")
            db.session.add(user)
            db.session.flush()
    ...
    def test_balance_validation(self, app):
        """Test account balance validation"""
        with app.app_context():
            user = User(microsoft_id="test", email="test@test.com", name="Test")
            db.session.add(user)
            ...
            assert account.has_sufficient_funds(Decimal('350.00'))  # With overdraft
            assert not account.has_sufficient_funds(Decimal('400.00'))  # Exceeds overdraft
            
            # Test balance limits
            assert not account.would_exceed_limit(Decimal('200.00'))  # Would be 300, exceeds max
            assert account.would_exceed_limit(Decimal('200.00'))
    
    def test_update_balance(self, app):
        """Test balance update with validation"""
        with app.app_context():
            user = User(microsoft_id="test", email="test@test.com", name="Test")
            db.session.add(user)
            db.session.flush()
            ...
            # Valid update
            new_balance = account.update_balance(Decimal('50.00'))
            assert new_balance == Decimal('150.00')
            assert account.balance == Decimal('150.00')
            
            # Invalid update - exceeds maximum
            with pytest.raises(ValueError):
                account.update_balance(Decimal('200.00'))
            
            # Invalid update - exceeds overdraft
            with pytest.raises(ValueError):
                account.update_balance(Decimal('-400.00'))

class TestTransactionModel:
    """Test cases for Transaction model"""
    
    def test_create_transfer(self, app):
        """Test creating a peer-to-peer transfer"""
        with app.app_context():
            sender = User(microsoft_id="sender", email="sender@test.com", name="Sender")
            recipient = User(microsoft_id="recipient", email="recipient@test.com", name="Recipient")
            db.session.add_all([sender, recipient])
            ...
            assert transaction.transaction_type == TransactionType.TRANSFER
            assert transaction.category == "Lunch"
            assert transaction.note == "Shared lunch cost"
            assert transaction.is_transfer()
            assert not transaction.is_event_contribution()
    
    def test_create_event_contribution(self, app):
        """Test creating an event contribution"""
        with app.app_context():
            creator = User(microsoft_id="creator", email="creator@test.com", name="Creator")
            contributor = User(microsoft_id="contrib", email="contrib@test.com", name="Contributor")
            db.session.add_all([creator, contributor])
            db.session.flush()
            ...
            assert transaction.event_id == event.id
            assert transaction.amount == Decimal('25.00')
            assert transaction.is_event_contribution()
...
class TestEventAccountModel:
    ...
    def test_create_event_account(self, app):
        ...
        with app.app_context():
            ...
            assert event.id is not None
            assert event.creator_id == creator.id
            assert event.name == "Team Building"
            assert event.target_amount == Decimal('500.00')
            assert event.status == EventStatus.ACTIVE
            assert event.is_active()
            assert event.can_receive_contributions()
    
    def test_event_contributions_calculation(self, app):
        """Test event contributions calculation"""
        with app.app_context():
            creator = User(microsoft_id="creator", email="creator@test.com", name="Creator")
            contributor1 = User(microsoft_id="c1", email="c1@test.com", name="Contributor 1")
            contributor2 = User(microsoft_id="c2", email="c2@test.com", name="Contributor 2")
            ...
            # Add contributions
            contrib1 = Transaction.create_event_contribution(contributor1.id, event.id, Decimal('30.00'))
            contrib2 = Transaction.create_event_contribution(contributor2.id, event.id, Decimal('20.00'))
            contrib1.mark_as_processed()
            contrib2.mark_as_processed()
            
            db.session.add_all([contrib1, contrib2])
            db.session.commit()
            
            # Refresh event to get updated contributions
            db.session.refresh(event)
            
            assert event.total_contributions == Decimal('50.00')
            assert event.get_progress_percentage() == 50.0
            assert event.get_remaining_amount() == Decimal('50.00')
            assert event.get_contributor_count() == 2
...
class TestMoneyRequestModel:
    ...
    def test_create_money_request(self, app):
        """Test creating a money request"""
        with app.app_context():
            requester = User(microsoft_id="req", email="req@test.com", name="Requester")
            recipient = User(microsoft_id="rec", email="rec@test.com", name="Recipient")
            db.session.add_all([requester, recipient])
            db.session.flush()
            
            request = MoneyRequest.create_request(
                requester_id=requester.id,
                recipient_id=recipient.id,
            ...
            assert request.requester_id == requester.id
            assert request.recipient_id == recipient.id
            assert request.amount == Decimal('75.00')
            assert request.note == "Lunch money"
            assert request.is_pending()
            assert request.can_be_responded_to()
    
    def test_money_request_responses(self, app):
        """Test money request approval and decline"""
        with app.app_context():
            requester = User(microsoft_id="req", email="req@test.com", name="Requester")
            recipient = User(microsoft_id="rec", email="rec@test.com", name="Recipient")
            db.session.add_all([requester, recipient])
            ...
            # Test approval
            request1 = MoneyRequest.create_request(requester.id, recipient.id, Decimal('50.00'))
            request1.approve()
            
            assert request1.is_approved()
            assert not request1.can_be_responded_to()
            assert request1.responded_at is not None
            
            # Test decline
            request2 = MoneyRequest.create_request(requester.id, recipient.id, Decimal('25.00'))
            request2.decline()
            
            assert request2.is_declined()
            assert not request2.can_be_responded_to()
            assert request2.responded_at is not None
...
class TestAuditLogModel:
    ...
    def test_log_user_action(self, app):
        ...
        with app.app_context():
            ...
            log_entry = AuditLog.log_user_action(
                user_id=user.id,
            ...
            assert log_entry.entity_id == "test-123"
            assert log_entry.new_values == {"test": "value"}
            assert log_entry.ip_address == "127.0.0.1"
    
    def test_log_system_event(self, app):
        """Test logging system events"""
        with app.app_context():
            log_entry = AuditLog.log_system_event(
            ...
            assert log_entry.action_type == "SYSTEM_STARTUP"
            assert log_entry.entity_type == "System"
            assert log_entry.new_values == {"version": "1.0.0"}